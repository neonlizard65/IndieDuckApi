...
class Bundle{
    ...
    private $table_name = 'Bundle'; //таблица
    ...
    function getBundleByID(){
        $query = "SELECT g.BundleID, p.Name, p.ReleaseDate, p.DeveloperId, p.PublisherId, p.Discount, p.ShortBio, p.About,
        p.FranchiseId, f.FranchiseName, p.AgeRatingESRB FROM " . $this->table_name . " g 
        INNER JOIN Product p ON g.BundleID = p.ProductID 
        LEFT JOIN Franchise f ON p.FranchiseId = f.FranchiseID
        WHERE g.BundleID =:BundleID";
        // подготовка запроса 
    ...
    function getBundleBySearch($search){
        $query = "SELECT g.BundleID, p.Name, p.ReleaseDate, p.DeveloperId, p.PublisherId, p.Discount, p.ShortBio, p.About,
        p.FranchiseId, f.FranchiseName, p.AgeRatingESRB FROM " . $this->table_name . " g 
        INNER JOIN Product p ON g.BundleID = p.ProductID 
        LEFT JOIN Franchise f ON p.FranchiseId = f.FranchiseID
    ...
    function getBundleByDiscount($mindiscount){
        $query = "SELECT g.BundleID, p.Name, p.ReleaseDate, p.DeveloperId, p.PublisherId, p.Discount, p.ShortBio, p.About,
        p.FranchiseId, f.FranchiseName, p.AgeRatingESRB FROM " . $this->table_name . " g 
        INNER JOIN Product p ON g.BundleID = p.ProductID 
        LEFT JOIN Franchise f ON p.FranchiseId = f.FranchiseID
    ...
    function getBundleByDates($date1, $date2){
        $query = "SELECT g.BundleID, p.Name, p.ReleaseDate, p.DeveloperId, p.PublisherId, p.Discount, p.ShortBio, p.About,
        p.FranchiseId, f.FranchiseName, p.AgeRatingESRB FROM " . $this->table_name . " g 
        INNER JOIN Product p ON g.BundleID = p.ProductID 
        LEFT JOIN Franchise f ON p.FranchiseId = f.FranchiseID
    ...
    function delete(){
        ...
        $query = "DELETE FROM " . $this->table_name . " WHERE BundleID =:BundleID";
        $query2 = "DELETE FROM Product WHERE ProductID =:BundleID";
        ...
        $stmt->bindParam(":BundleID", $this->BundleID);
        $stmt2->bindParam(":BundleID", $this->BundleID);
        // выполняем запрос 
        if ($stmt->execute() && $stmt2->execute()) {
            return true;
        }
